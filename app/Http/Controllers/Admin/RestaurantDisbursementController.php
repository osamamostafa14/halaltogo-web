...
class RestaurantDisbursementController extends Controller
{
    ...
    public function status(Request $request)
    {
        ...
        foreach ($disbursements as $disbursement){
            ...
            if($request->status == 'completed'){
                ...
                $wallet->decrement('pending_withdraw', $disbursement['disbursement_amount']);
                // if($wallet->pending_withdraw  >=   $disbursement['disbursement_amount']){
                //     // $wallet->increment('total_withdrawn', $disbursement['disbursement_amount']);
                //     } else{
                //         return response()->json([
                //             'status' => 'error',
                //             'message'=> translate('messages.Balance_mismatched_for').' '.$disbursement->restaurant?->name,
                //         ]);
                //     }

                // RestaurantWallet::where('vendor_id', $disbursement->restaurant->vendor_id)->decrement('pending_withdraw', $disbursement['disbursement_amount']);
            }
    ...
    public function statusById($id,$status)
    {
        ...
        if($status == 'completed'){
            ...
            //     // $wallet->increment('total_withdrawn', $disbursement['disbursement_amount']);
            //     } else{
            //         Toastr::error(translate('messages.Balance_mismatched_for').' '.$disbursement->restaurant?->name);
            //         return back();
            //     }
    ...
    public function generate_disbursement()
    {
        ...
        foreach ($restaurants as $restaurant){
            if(isset($restaurant->vendor->wallet)){
                ...
                $disbursement_amount = ($total_earning>($total_withdraw+$total_cash_in_hand))?($total_earning - ($total_withdraw+$total_cash_in_hand)):0;
